-- sb_companies | Crafting Recipes
-- Moved from sb_mechanic_v2/shared/recipes.lua
-- Company workers use these to craft parts from raw materials
-- All recipe definitions organized by bench type

Recipes = {}

-- ===================================================================
-- BENCH DEFINITIONS (now at company production areas)
-- ===================================================================
Recipes.Benches = {
...
-- Each company has specific bench types available
Recipes.CompanyBenches = {
    santos_metal   = { 'metal_bench', 'engine_bench' },
    pacific_chem   = { 'fluid_station', 'tire_machine', 'metal_bench' },
    ls_electronics = { 'electronics_bench' },
}
...
--- Get recipes available at a specific bench filtered by skill level
...
--- Get all recipes a company can produce (based on their bench types)
function Recipes.GetCompanyRecipes(companyId)
    local benches = Recipes.CompanyBenches[companyId]
    if not benches then return {} end

    local result = {}
    for _, benchId in ipairs(benches) do
        local benchRecipes = Recipes.ByBench[benchId]
